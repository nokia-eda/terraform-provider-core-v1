...
package datasource_branch_diff_summary
...
func BranchDiffSummaryDataSourceSchema(ctx context.Context) schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			"branch_head_hash": schema.StringAttribute{
				Computed:            true,
				Description:         "Git commit hash describing the current state of this branch (head of branch). Should be passed to subsequent diff requests for the current state.",
				MarkdownDescription: "Git commit hash describing the current state of this branch (head of branch). Should be passed to subsequent diff requests for the current state.",
			},
			"changed_crs": schema.ListNestedAttribute{
				...
				Computed:            true,
				Description:         "Paginated list of changed CRs grouped by GVK.",
				MarkdownDescription: "Paginated list of changed CRs grouped by GVK.",
			},
			"changed_crs_count": schema.Int64Attribute{
				Computed:            true,
				Description:         "Number of CRs that differ between this branch and production.",
				MarkdownDescription: "Number of CRs that differ between this branch and production.",
			},
			"cursor": schema.Int64Attribute{
				Optional:            true,
				Computed:            true,
				Description:         "When gvk is set with limit: offset into that category's names; omit for first page. Ignored when gvk is omitted.",
				MarkdownDescription: "When gvk is set with limit: offset into that category's names; omit for first page. Ignored when gvk is omitted.",
			},
			"gvk": schema.StringAttribute{
				Optional:            true,
				Computed:            true,
				Description:         "When set, returns only a single category for that GVK. Must be valid JSON for GroupVersionKind (URL-encoded). Omit to return all categories.",
				MarkdownDescription: "When set, returns only a single category for that GVK. Must be valid JSON for GroupVersionKind (URL-encoded). Omit to return all categories.",
			},
			"limit": schema.Int64Attribute{
				Optional:            true,
				Computed:            true,
				Description:         "Max names per category per page. Omit or 0: return all names (no pagination). Max 1000.",
				MarkdownDescription: "Max names per category per page. Omit or 0: return all names (no pagination). Max 1000.",
			},
			"search": schema.StringAttribute{
				Optional:            true,
				Computed:            true,
				Description:         "Filter names by case-insensitive substring match on name, namespace, or \"name (namespace)\" display.",
				MarkdownDescription: "Filter names by case-insensitive substring match on name, namespace, or \"name (namespace)\" display.",
			},
			"visible_crs_count": schema.Int64Attribute{
				Computed:            true,
				Description:         "Number of changed CRs to which the user has access. Less than changedCrsCount when some are omitted due to permission.",
				MarkdownDescription: "Number of changed CRs to which the user has access. Less than changedCrsCount when some are omitted due to permission.",
			},
		},
	}
}

type BranchDiffSummaryModel struct {
	BranchHeadHash  types.String `tfsdk:"branch_head_hash"`
	...
	VisibleCrsCount types.Int64  `tfsdk:"visible_crs_count"`
}
